...
export default class CalcularParcelamento{
    executar(
        ...
        const totalJuros = this.calcularJurosCompostos(valor, taxaJuros, qtdeParcelas)

        return {
            valorParcela: this.comDuasCasasDecimais(totalJuros / qtdeParcelas),
            valorTotal: this.comDuasCasasDecimais(totalJuros),
            qtdeParcelas,
            taxaJuros,
        }
    }
